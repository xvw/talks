...
let display flag (elt : Dom_html.element Js.t) =
  if flag then begin
    elt##.classList##remove(Js.string "inactive-slide");
    elt##.classList##add(Js.string "active-slide")
  end
  else begin
    elt##.classList##add(Js.string "inactive-slide");
    elt##.classList##remove(Js.string "active-slide")
  end
...
let init_mask () =
  let _ = List.iter (display false) retreive_diapositives in 
  let _ = display true (current_div ())
  in List.iteri append_label (retreive_diapositives)
                    
let next_mask () =
  let _ = cursor := (bound cmin cmax (!cursor + 1)) in
  let _ = List.iter (display false) retreive_diapositives
  in display true (current_div ());
     set_hash ()

let prev_mask () =
  let _ = cursor := (bound cmin cmax (!cursor - 1)) in
  let _ = List.iter (display false) retreive_diapositives
  in display true (List.nth retreive_diapositives !cursor);
     set_hash ()
